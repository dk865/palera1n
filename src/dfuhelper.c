...
#define IS_APPLE_HOME  (IS_APPLE_HOME1 || IS_APPLE_HOME2)
...
void* connected_recovery_mode(struct irecv_device_info* info) {
    ...
    // Add 3-second "Get Ready" countdown for all devices
    ...
    if (IS_APPLETV) {
        if (IS_APPLE_TV_HD) {
            ...
            step(4, 0, "Put device in upside down orientation", conditional, ecid);
        }
    } else if (cpid != 0x8012) {
